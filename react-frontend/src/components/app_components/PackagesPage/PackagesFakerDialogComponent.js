...
import packagesFakerFactory from "./packagesFakerFactory";
...
const PackagesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = packagesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("packages").create(elem));
...
export default connect(mapState, mapDispatch)(PackagesFakerDialogComponent);
