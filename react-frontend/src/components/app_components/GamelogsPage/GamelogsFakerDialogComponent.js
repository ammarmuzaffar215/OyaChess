...
import gamelogsFakerFactory from "./gamelogsFakerFactory";
...
const GamelogsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = gamelogsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("gamelogs").create(elem));
...
export default connect(mapState, mapDispatch)(GamelogsFakerDialogComponent);
