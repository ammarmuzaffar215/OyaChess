...
const EnrollmentsPage = (props) => {
  ...
  useEffect(() => {
    ...
    client
      ...
      .find({
        query: {
          $limit: 10000,
          packageId: urlParams.singlePackagesId,
      ...
      .catch((error) => {
        ...
        props.alert({
          title: "Enrollments",
          type: "error",
          message: error.message || "Failed get Enrollments",
        });
      });
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("enrollments").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
