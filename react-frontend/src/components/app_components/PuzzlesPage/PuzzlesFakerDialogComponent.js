...
import puzzlesFakerFactory from "./puzzlesFakerFactory";
...
const PuzzlesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = puzzlesFakerFactory(props.user,fakerCount,userIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("puzzles").create(elem));
...
export default connect(mapState, mapDispatch)(PuzzlesFakerDialogComponent);
