...
import purchasesFakerFactory from "./purchasesFakerFactory";
...
const PurchasesFakerDialogComponent = (props) => {
...
client.service("items")
    ...
    const onRun = async () => {
        let fakeData = purchasesFakerFactory(props.user,fakerCount,userIdIds,itemIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("purchases").create(elem));
...
export default connect(mapState, mapDispatch)(PurchasesFakerDialogComponent);
