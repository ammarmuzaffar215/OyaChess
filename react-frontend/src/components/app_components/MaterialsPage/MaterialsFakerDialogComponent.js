...
import materialsFakerFactory from "./materialsFakerFactory";
...
const MaterialsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [packageIdIds, setpackageIdIds] = useState();
    ...
    const onRun = async () => {
        let fakeData = materialsFakerFactory(props.user,fakerCount,packageIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("materials").create(elem));
...
export default connect(mapState, mapDispatch)(MaterialsFakerDialogComponent);
