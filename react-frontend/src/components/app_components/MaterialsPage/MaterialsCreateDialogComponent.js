...
const MaterialsCreateDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [packageId], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            title: _entity?.title,packageId: _entity?.packageId?._id,videoUrl: _entity?.videoUrl,materials: _entity?.materials,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Materials updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Materials" });
        }
        setLoading(false);
    };
    ...
    const setId = (id) => { setValByKey("materials", id);  };
    ...
    return (
        <Dialog header="Create Materials" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="materials-create-dialog-component">
...
<div className="col-12 field">
                <span className="align-items-center">
                    ...
                    <UploadFilesToS3 type={'create'} user={props.user} id={urlParams.id} serviceName="materials" onUploadComplete={setId} onFileLoaded={onFileLoaded}/>
                </span>
                <small className="p-error">
